...
class MlxWhisperTranscriber:
    """Apple Silicon (Metal/MPS) icin mlx-whisper tabanli transkripsiyon.

    faster-whisper'in kullandigi CTranslate2 motoru MPS desteklemedigi
    (yalnizca cuda/cpu) icin bu ayri backend Apple'in kendi MLX framework'unu
    kullanir. Ayni transcribe(audio_24k, start_time) arayuzunu saglar, boylece
    websocket_endpoint tarafi hangi backend'in secildigini bilmeye ihtiyac
    duymaz."""
    ...
    def __init__(self) -> None:
        ...
        self.model_repo = os.environ.get(
            "WHISPER_MLX_MODEL", "mlx-community/whisper-large-v3-turbo"
        )
        self.language = os.environ.get("WHISPER_LANGUAGE", "tr")
        self.temperature = _env_float("WHISPER_TEMPERATURE", 0.0)
        self._warmup()
    ...
    def _warmup(self) -> None:
        ...
        logger.info("MLX Whisper warmup finished in %.3fs", time.perf_counter() - t0)
    ...
    def transcribe(self, audio_24k: np.ndarray, start_time: float) -> list[TranscribedWord]:
        ...
        for segment in result.get("segments", []):
            fallback_text_parts.append(str(segment.get("text", "")).strip())
            for word in segment.get("words") or []:
                ...
                if text:
                    words.append(
                        TranscribedWord(
                            text=text,
                            start_time=start_time + max(0.0, float(word.get("start", 0.0))),
                        )
                    )

        if not words:
            # mlx-whisper doesn't always return word-level timestamps.
            text = " ".join(part for part in fallback_text_parts if part).strip()
            words = [
                TranscribedWord(text=word, start_time=start_time)
                for word in text.split()
        ...
        audio_duration = audio_24k.size / SAMPLE_RATE
        logger.info(
            "MLX transcribed %.2fs audio in %.3fs, rtf=%.3f, language=%s, text=%r",
            audio_duration,
            elapsed,
            elapsed / max(audio_duration, 1e-6),
            self.language,
...
def load_model() -> None:
    backend = os.environ.get("STT_BACKEND", "faster-whisper").lower()
    if backend == "mlx":
        app.state.transcriber = MlxWhisperTranscriber()
    else:
