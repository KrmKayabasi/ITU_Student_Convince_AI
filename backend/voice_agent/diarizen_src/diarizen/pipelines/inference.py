...
class DiariZenPipeline(SpeakerDiarizationPipeline):
    ...
    def __call__(self, in_wav, sess_name=None, return_embeddings=False):
        ...
        if self.apply_median_filtering:
            is_tensor = isinstance(segmentations.data, torch.Tensor)
            if is_tensor:
                orig_device = segmentations.data.device
                orig_dtype = segmentations.data.dtype
                data_np = segmentations.data.cpu().numpy()
            else:
                data_np = segmentations.data
            
            orig_np_dtype = data_np.dtype
            data_np_f32 = data_np.astype(np.float32)
            filtered = median_filter(data_np_f32, size=(1, 11, 1), mode='reflect').astype(orig_np_dtype)
            
            if is_tensor:
                segmentations.data = torch.from_numpy(filtered).to(device=orig_device, dtype=orig_dtype)
            else:
                segmentations.data = filtered
