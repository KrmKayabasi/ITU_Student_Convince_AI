...
# LANGUAGE AND TTS
Your answer will be passed directly to a text-to-speech system. Use plain spoken text.
When speaking or switching to French, or opening to a quote in French, always use
French guillemets « ». Never put a ':' before a "«".
...
LanguageCode = Literal["en", "fr", "en/fr", "fr/en", "tr"]
LANGUAGE_CODE_TO_INSTRUCTIONS: dict[LanguageCode | None, str] = {
    ...
    "tr": "Türkçe konuş. Kullanıcı açıkça başka bir dil istemedikçe her zaman Türkçe cevap ver. Cevapların kısa, doğal ve konuşma diline uygun olsun. Hey, um, uh gibi İngilizce dolgu kelimeleri kullanma. Kendine isim veya kişilik uydurma.",
}


# A trimmed system prompt for the Turkish low-latency pipeline.
#
# The default _SYSTEM_PROMPT_TEMPLATE is ~1,000 Turkish tokens (about half of a 2048
# context window) and mixes French-guillemets rules, unmute/Kyutai marketing, filler-word
# advice, and "respond in the language the user is speaking" — which conflicts with the
# forced-Turkish instruction and dilutes a small LLM's attention. This compact variant
# keeps only what the Turkish smalltalk test actually needs: speech/TTS format, the
# Turkish language directive, transcription-error handling, and the per-character
# additional instructions.
_TURKISH_SYSTEM_PROMPT_TEMPLATE = """
Sen bir sesli sohbet asistanısın. Kullanıcının konuşması sesli metne çevrilir (STT) ve
senin cevabın seslendirilir (TTS). Cevaplarını düz konuşma metni olarak yaz; emoji, yıldız
işareti veya "..." gibi yazıya özgü işaretleri kullanma. Her şeyi olduğu gibi telaffuz et.
...
# KONUŞMA TANIMA HATALARI
Kullanıcının metninde STT kaynaklı hatalar olabilir. Anlamsız görünen bir şey gördüğünde
bunun bir transkripsiyon hatası olabileceğini hesaba kat. Açıkça bir hata ise ve benzeyen
doğru kelimeyi tahmin edebiliyorsan, kullanıcıya sormak yerine tahmin et.

# SESSİZLİK
Kullanıcı "..." yazarsa bu bir süredir konuşmadığı anlamına gelir; hâlâ orada olup
olmadığını sorabilir veya sessizliği dolduracak kısa bir şey söyle. Üç kere cevap vermezse
kısa bir vedalaşma mesajı yaz ve mesajını "Hoşça kal!" ile bitir.

# KİM OLDUĞUN
Bu site "unmute dot SH". Sen Kyutai tarafından geliştirilen, seslenebilen modüler bir
AI'sın. Sistem üç parçadan oluşur: bir sesli metne çeviri modeli ("kulaklar"), bir LLM
("beyin", model: "{llm_name}") ve bir metni sese çeviri modeli ("ağız"). STT ve TTS açık
kaynaktır, kyutai dot org adresinde bulunabilir.
"""
...
def _format_system_prompt(
    ...
    """Pick the right template for the language and format it.

    Uses the compact Turkish template for `language == "tr"` (to save context and
    avoid the conflicting English/French directives in the default template), and
    the original template for everything else.
    """
    if language == "tr":
        return _TURKISH_SYSTEM_PROMPT_TEMPLATE.format(
...
_VALID_LANGUAGE_CODES = {"en", "fr", "en/fr", "fr/en", "tr"}


def get_default_language() -> LanguageCode | None:
    """Default language for the initial system prompt, before the frontend sends
    `session.update`. Set via `UNMUTE_DEFAULT_LANGUAGE` (e.g. "tr" so a Turkish
    pipeline speaks Turkish from the very first reply). Unset/invalid falls back
    to None, which is the original English-with-some-French default.
    """
    raw = os.environ.get("UNMUTE_DEFAULT_LANGUAGE")
    if raw is None:
        return None
    if raw not in _VALID_LANGUAGE_CODES:
        logger.warning(
            "UNMUTE_DEFAULT_LANGUAGE=%r is not a valid language code "
            "(expected one of %s); ignoring.",
            raw,
            sorted(_VALID_LANGUAGE_CODES),
        )
        return None
...
SMALLTALK_INSTRUCTIONS_TR = """
{additional_instructions}

# BAĞLAM
Şu an {current_time} ({timezone}).

# SOHBETİ BAŞLAT
Kullanıcının mesajına bir selam ve sohbet başlatacak bir şeyle cevap ver. Örneğin
{conversation_starter_suggestion}.
"""

CONVERSATION_STARTER_SUGGESTIONS_TR = [
    "gününün nasıl geçtiğini sor",
    "ne üzerinde çalıştığını sor",
    "şu an ne yaptığını sor",
    "ilgi alanlarını veya hobilerini sor",
    "eğlenceli bir konu öner",
    "sormak istediği bir şey olup olmadığını sor",
    "bugün neye motive olduğunu sor",
    "en sevdendiği dinlenme şeklini sor",
    "öğrenmek istediği yeni bir beceri olup olmadığını sor",
    "bu hafta neye dört gözle beklediğini sor",
    "son zamanlarda sevdiği bir kitap veya filmi sor",
    "son zamanlarda gülümseten bir şey sor",
]
...
class SmalltalkInstructions(BaseModel):
    ...
    def make_system_prompt(
        ...
        if self.language == "tr":
            ...
            smalltalk_block = SMALLTALK_INSTRUCTIONS.format(
                additional_instructions=additional_instructions,
                current_time=datetime.datetime.now().strftime("%A, %B %d, %Y at %H:%M"),
                timezone=datetime.datetime.now().astimezone().tzname(),
                conversation_starter_suggestion=random.choice(
        ...
        return _format_system_prompt(
            language=self.language,
            language_instructions=LANGUAGE_CODE_TO_INSTRUCTIONS[self.language],
            additional_instructions=smalltalk_block,
            llm_name=get_readable_llm_name(),
        )
...
class UnmuteExplanationInstructions(BaseModel):
    ...
    def make_system_prompt(self) -> str:
        # Always English; UnmuteExplanationInstructions has no `language` field.
        return _format_system_prompt(
            language="en",
