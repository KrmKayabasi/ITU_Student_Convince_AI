...
from unmute.llm.system_prompt import ConstantInstructions, Instructions, get_default_language
...
class Chatbot:
    def __init__(self):
        ...
        # Start with the pipeline's default language (e.g. UNMUTE_DEFAULT_LANGUAGE=tr
        # for Turkish) so the very first reply is in the right language, before the
        # frontend's session.update arrives and calls set_instructions().
        self.chat_history: list[dict[Any, Any]] = [
            {
                "role": "system",
                "content": ConstantInstructions(
