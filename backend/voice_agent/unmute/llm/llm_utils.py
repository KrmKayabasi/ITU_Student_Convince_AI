...
class VLLMStream:
    def __init__(
        ...
        self.max_tokens = int(os.environ.get("KYUTAI_LLM_MAX_TOKENS", "80"))
        # Sampling knobs. Defaults are chosen to reduce Gemma's tendency to repeat;
        # set the env var to a non-positive value to disable a knob (vLLM/OpenAI treat
        # the absence of the field as "no effect" rather than 0).
        self.top_p = float(os.environ.get("KYUTAI_LLM_TOP_P", "0.9"))
        self.repetition_penalty = float(
            os.environ.get("KYUTAI_LLM_REPETITION_PENALTY", "1.1")
        )
    ...
    async def chat_completion(
        ...
        # top_p is a standard OpenAI API param, pass it as a top-level kwarg.
        if self.top_p and self.top_p > 0:
            extra_kwargs["top_p"] = self.top_p

        # repetition_penalty is a vLLM/HuggingFace-specific param that the OpenAI
        # SDK doesn't know about (it rejects unknown kwargs). Route it through
        # `extra_body`, which the SDK forwards verbatim to the server. vLLM reads
        # it from there. See https://platform.openai.com/docs/api-reference/more
