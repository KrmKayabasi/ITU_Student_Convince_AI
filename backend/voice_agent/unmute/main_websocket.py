...
async def _get_health(
    ...
    cascaded_url = os.environ.get("CASCADED_SERVER_URL", "http://host.docker.internal:8002" if os.path.exists("/.dockerenv") else "http://localhost:8002")
    is_cascaded_up = await asyncio.to_thread(
        _check_server_status, cascaded_url + "/last_turn"
    )
    return HealthStatus(
        tts_up=is_cascaded_up,
        stt_up=is_cascaded_up,
        llm_up=is_cascaded_up,
        voice_cloning_up=is_cascaded_up,
    )
