...
from unmute.llm.system_prompt import LanguageCode, SmalltalkInstructions
    ...
    await websocket.send(
        ora.SessionUpdate(
            session=ora.SessionConfig(
                instructions=SmalltalkInstructions(language=language),
                voice=voice,
...
async def _main(
    ...
    async with websockets.connect(
        ...
        emit_task = asyncio.create_task(
            emit_loop(websocket, audio_to_emit, voice, language)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--language",
        type=str,
        choices=["en", "fr", "en/fr", "fr/en", "tr"],
        default=None,
        help="Language instruction to send to the LLM.",
    )
