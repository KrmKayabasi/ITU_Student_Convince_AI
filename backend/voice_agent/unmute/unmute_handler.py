...
class UnmuteHandler(AsyncStreamHandler):
    def __init__(self) -> None:
        ...
        # Words received from the STT since the last bot generation. Used to gate
        # end-of-turn: we only declare the user finished if they actually spoke
        # something in this turn. This prevents the bot from responding to a
        # momentary RMS dip (a breath) before any real utterance was transcribed.
        # Critical for utterance-level STT backends (e.g. the Turkish Whisper
        # adapter) whose binary pause signal flips on the instant energy drops.
        self.words_received_this_turn = 0
    ...
    async def _generate_response(self):
        ...
        # The bot is taking the floor: reset the per-turn word counter so the
        # next user turn must produce real words before end-of-turn can fire.
        self.words_received_this_turn = 0
    ...
    def determine_pause(self) -> bool:
        ...
        if stt.pause_prediction.value > 0.6:
            # Only end the user's turn if they have actually spoken words in this
            # turn. Without this gate, a momentary energy dip (a breath, brief
            # silence between words) can flip the pause signal to 1.0 before any
            # real utterance was transcribed, making the bot respond to nothing
            # and fragment the user's turn. See self.words_received_this_turn.
            if self.words_received_this_turn == 0:
                return False
    ...
    async def _stt_loop(self, stt: SpeechToText):
        try:
            async for data in stt:
                ...
                # Count real words received this turn so determine_pause() can
                # gate end-of-turn on the user having actually spoken.
                self.words_received_this_turn += len(data.text.split())
    ...
    async def _tts_loop(self, tts: TextToSpeech, generating_message_i: int):
        ...
        self.waiting_for_user_start_time = self.audio_received_sec()
        # The user's turn is starting fresh: reset the per-turn word counter so
        # end-of-turn can't fire on a stale word count from the previous turn.
        self.words_received_this_turn = 0
