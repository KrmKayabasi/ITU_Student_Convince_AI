"""
FastAPI router for speaker enrollment and management.
...
Mounted by both the orchestrator and speech_backend servers.
"""
...
def create_speaker_router(speaker_manager) -> APIRouter:
    ...
    async def enroll_speaker(
        ...
        Body: raw PCM float32 bytes (little-endian), 16kHz, mono, 3-10 seconds.
        """
    ...
    async def verify_speaker(
        ...
        Body: raw PCM float32 bytes (little-endian), 16kHz, mono.
        """
        if not speaker_manager.is_ready:
            raise HTTPException(status_code=503, detail="Speaker manager not initialized")
    ...
    async def speaker_health():
        ...
        return {
            ...
            "target_speaker_id": speaker_manager.target_speaker_id,
            "profiles_count": (
                speaker_manager._db.speaker_count
                if speaker_manager._db
                else 0
            ),
        }
